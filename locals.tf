...
locals {
    ...
    compartments_dependency = merge({for k, v in coalesce(var.compartments_dependency,{}) : k => {"id" : v.id}}, {for k, v in (length(module.oci_lz_compartments) > 0 ? module.oci_lz_compartments[0].compartments : {}) : k => {"id" : v.id}})
}